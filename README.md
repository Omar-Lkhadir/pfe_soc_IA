...
- **`hist_gradient_boosting_palier2/`** — Palier 2 : catégorisation des attaques détectées par le palier 1 en 5 classes (`brute_force`, `port_scanning`, `suspicious_activity`, `intrusion_reseau`, `acces_non_autorise`), même algorithme que le palier 1 (`HistGradientBoostingClassifier`). Ce dossier s'appelait `random_forest/` à l'origine du projet ; renommé pour refléter l'algorithme réellement utilisé (`RandomForestClassifier` a été testé puis écarté, voir la présentation du projet).
